...
i2s_config_t i2s_config_rx  = {
  ...
  dma_buf_count: 2,                            // number of buffers, 128 max.
  dma_buf_len: BUFFER_LEN                          // size of each buffer
};
...
void loop() {
  size_t bytes_read = 0;
  if ((i2s_read(I2S_NUM_1, (void *)audio_buffer,BUFFER_LEN,&bytes_read,10000)==ESP_OK) 
			  && (bytes_read > 0)){
	 printBarForGraph(abs(audio_buffer[1])); //sample 0 may be corrupted
	 fwrite(audio_buffer,sizeof(uint32_t),BUFFER_LEN,f);
	 num_samples++;
   }
   /*
